...
export const JournalProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const addTrade = async (tradeData: Omit<NewTrade, 'user_id'>) => {
    ...
    try {
      ...
      if (err?.code === 'PGRST303' || err?.message?.includes('JWT expired')) {
        ...
        setTimeout(() => signOut(), 3000);
        // Não relança o erro para evitar toast duplicado no formulário
      } else {
        // Para outros erros, relança para que o formulário possa tratá-los
        throw err;
      }
    }
  };
  ...
  const { stats, equityCurve } = useMemo(() => {
    ...
    const equityCurveData = trades.map(trade => {
      ...
      return { day: format(new Date(trade.trade_date), 'dd/MM'), balance: currentBalance };
    });
  ...
  const value = {
    trades: [...trades].sort((a, b) => new Date(b.trade_date).getTime() - new Date(a.trade_date).getTime()), // Descending for table display
    stats,
    equityCurve,
    loading,
    error,
